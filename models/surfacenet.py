...
class Backbone(nn.Module):
    ...
    def __init__(self):
        ...
        self.decoder = nn.ModuleList([
            upsample_conv(in_channels=256+3+1,
                          out_channels=self.out_feat),
            upsample_conv(in_channels=256+3+1,
                          out_channels=self.out_feat),
            upsample_conv(in_channels=256+3+1,
                          out_channels=self.out_feat),
            same_conv(in_channels=256+3+1,
                      out_channels=self.out_feat)
        ])
        
    def forward(self, x, sketch_map, **kwargs):
        
        feat = self.encoder(x)
        feat = self.bottleneck(feat)
        
        for layer in self.decoder:
            pooled_sketch = F.interpolate(
                sketch_map, feat.shape[-2], mode="bilinear", align_corners=False)
            pooled_x = F.interpolate(
                x, feat.shape[-2], mode="bilinear", align_corners=False)
            feat = layer(torch.cat((feat,pooled_x, pooled_sketch), 1))
...
class SurfaceNet(nn.Module):
    ...
    def forward(self, x, sketch_map=None, pad_image=False):
        _, _, h, w = x.shape
        if sketch_map is None:
            sketch_map = torch.randn(x.size(0), 1, x.size(2), x.size(3)).to(x.device)

        _,_, h_s,w_s=sketch_map.shape

        x = F.pad(x, pad=(32, 32, 32, 32), mode="replicate") if pad_image else x
        sketch_map=F.pad(sketch_map,pad=(32,32,32,32), mode="replicate") if pad_image else sketch_map
        
        out = self.encoder(x, sketch_map)
